...
if (isset($_REQUEST['elect_submit'])) {
	...
	if ($n == 1) {
		...
		echo "Product is not Upload";
	}
}
// Delete
if (@$_REQUEST['delete'] == 'yes') {
	$productid = $_REQUEST['id'];
	$n = iud("DELETE FROM `items` WHERE elecid='$productid'");
	if ($n == 1) {
...
`image`='$name',`discription`='$udiscription',`price`='$uprice',`publisher`='$upublisher' WHERE productid='$uid'";
	...
	if (move_uploaded_file($tmp_name, "images/$name")) {
		...
		echo "<script>alert('Product is not Update ');
		 window.location='view_item_list.php';
		 </script>";
	}
}
